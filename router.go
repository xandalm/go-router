...
var ErrUnexpectedPanic = errors.New("router: goroutine execution interrupted by panic report")
...
func (ro *Router) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if errors = ro.crossMiddlewares(p, ww, rr); len(errors) > 0 {
		...
		if ro.meh == nil {
			...
			fmt.Fprintf(
				w,
				"Request interrupted by middleware's error report.\n"+
					"Error: %s\n"+
					"Stack Trace:\n%s\n"+
					"Message generated by the application's route handler package.",
				err.err,
				err.stack,
			)
	...
	defer func() {
		...
		if ro.meh == nil {
			...
			fmt.Fprintf(
				ww,
				"Request interrupted by the responsible application.\n"+
					"Cause: Panic\n"+
					"Received Value: %s\n"+
					"Stack Trace: %s\n"+
					"Message generated by the application's route handler package.",
			...
			ro.meh.Handle(ww, rr, ErrUnexpectedPanic)
		}
	}()
	h.ServeHTTP(ww, rr)
}
